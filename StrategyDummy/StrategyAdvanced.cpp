...
{
	//on actualise les nouvelles infos du tour : map, points, dicestock
	...
	if (gameTurn != turnCount) { // si on est a un nouveau tour de jeu, on change de strat
		innerGameTurn = 0; // gameTurn à l'intérieur d'un tour
		turnCount = gameTurn;
	...
	// contient en premier toutes les cases jouables, et en second un vector de toutes les cases attaquables depuis cette case

	// détermination si il faut aller en status startgame, à chaque tour interne
	auto all_cluster = All_cluster(Id);

	// si il y a plusieurs clusters on active le startgame
	...
	// détermination des autres stratégies
	if (needNewStatus) {
		status = choose_status();
	}
	else { innerGameTurn++; } // si c'est pas un nouveau tour on augmente le compteur de tour a l'intérieur des tours

	if (!InitTurn(playableAttackable)) { // initialise la variable playableAttackable et return fasle si il n'y a pas de case jouable
		return false;
	}
...
bool StrategyAdvanced::InitTurn(std::vector<std::pair<pSCell, std::vector<pSCell>>>& playableAttackable)
{
	...
	// contient en first toutes les cases jouables, et en second un vector de toutes les cases attaquables depuis cette case

	// trouver tous les cases jouables
	for (unsigned i = 0; i < Map.nbCells; ++i) {
		if (Map.cells[i].infos.owner == Id && Map.cells[i].infos.nbDices > 1) { // cell à nous et + de 1 dé
			for (unsigned j = 0; j < Map.cells[i].nbNeighbors; ++j) { // au moins 1 case ennemie dans les voisins
				if (Map.cells[i].neighbors[j]->infos.owner != Id) {
					std::vector<pSCell> attackable;
					playableAttackable.push_back({ &Map.cells[i],attackable }); // on rajoute l'adresse des cells qui sont jouables
					break;
				}
			}
		}
	...
	// trouver toutes les cases attaquables depuis nos cases jouables
	bool emptyAttackable = true; // permet de savoir des qu'on a au moins une case attaquable
	for (auto& it : playableAttackable) { // on parcourt toutes les cases jouables
		for (unsigned i = 0; i < it.first->nbNeighbors; ++i) { // permet de trouver tous les cases ennemies
			if (it.first->neighbors[i]->infos.owner != Id) {
				it.second.push_back(it.first->neighbors[i]); // on rajoute l'adresse des cells qui sont attaquables
...
{
	// pathfinding mis en place à partir de l'algorithme de prim
	// à chaque passage, on ajoute la cellule avec le moins de dés sur le chemin depuis la case de départ
	// on réitère jusqu'à trouver l'arrivée

	std::vector<unsigned int> in_range_cell = {}; // les cells qu'on peut ajouter
	std::vector<unsigned int> added_cells = { iddepart }; // les cells qu'on a ajouté
	std::vector<unsigned int> path = { idarrive };
	std::map<unsigned int, unsigned int> pred; // couple idcell, predecesseur cell, le predecesseur est la cell depuis laquelle on ajoute la prochaine
	std::map<unsigned int, unsigned int> distance_dice; // couple idcell, nb de dés pour arriver à la cell

	// initialisation des variables
	for (unsigned int i = 0; i < Map.nbCells; ++i) {
		if (i != iddepart) {
			distance_dice[i] = 1000; // supposée infinie
		}
		else {
	...
	while (!find_arrival && !in_range_cell.empty()) { // tant qu'on a pas trouvé l'arrivée et qu'il reste des cellules que l'on peut ajouter
		...
		for (unsigned int i = 0; i < cell_added->nbNeighbors; ++i) {
			if (cell_added->neighbors[i]->infos.owner != Id || cell_added->neighbors[i]->infos.id == idarrive) { // cell a l'adversaire ou arrivée
				...
				if (already_added == added_cells.end()) { // cell pas deja ajoutée
					...
					else { // ancienne cell : mise à jour pred et distance
		...
		// mise à jour distance cell added
		...
		// mise à jour in_range_cell (delete la cell qu'on vient d'ajouter)
		...
		// mise à jour variable find_arrival
	...
	if (!find_arrival) { // on a pas réussi à atteindre la fin
		...
		// faire le path : remonter de id fin jusqu'à la cell initiale avec pred
		while (path.back() != iddepart)
		{
			// trouver le prédécesseur du dernier ajout du path
...
{
	// retourne un vecteur contenant les différents clusters du joueur avec l'Id owner
	// les différents clusters sont contenus dans des vecteurs
	// les cells sont représentées par leur id

	std::vector<std::vector<unsigned int>> all_cluster = {};

	// on cherche les cells qui appartiennent à owner
	for (unsigned int i = 0; i < Map.nbCells; i++) {
		if (Map.cells[i].infos.owner == owner) {
			if (all_cluster.empty()) { // premier tour de boucle
				all_cluster.push_back(Cluster(Map.cells[i].infos.id)); // on met le cluster appartenant à la première cell trouvée
			}
			else {
				// on vérifie que la cell n'est pas déja présente dans un cluster que l'on a déja identifié
				bool already_seen = false;
				for (auto cluster = all_cluster.begin(); cluster != all_cluster.end(); cluster++) { // parcours des différents cluster enregistrés
...
{
	// calcul de toutes les cases présentes dans un cluster à partir de l'Id d'une cell
	...
	for (unsigned int i = 0; i < currentcell->nbNeighbors; i++) { // parcours des voisins
		if (currentcell->neighbors[i]->infos.owner == ownercell) { // sélection des voisins avec le même owner
	...
	while (!non_visited_cells.empty()) {
		...
		for (unsigned int i = 0; i < currentcell->nbNeighbors; i++) { // parcours des voisins
			if (currentcell->neighbors[i]->infos.owner == ownercell) { // selections des voisins avec le même owner
				auto p = std::find(cluster.begin(), cluster.end(), currentcell->neighbors[i]->infos.id);
				if (p == cluster.end()) { // vérif pas deja ajouté
					cluster.push_back(currentcell->neighbors[i]->infos.id);
					non_visited_cells.push_back(currentcell->neighbors[i]);
				}
			}
		}
		non_visited_cells.erase(non_visited_cells.begin()); // on enlève la case visitée
	}
	return cluster;
}
...
bool StrategyAdvanced::Startgame(STurn* turn, std::vector<std::pair<pSCell, std::vector<pSCell>>>& playableAttackable)
{
	...
	// diriger les dés vers ce cluster

	auto all_cluster = All_cluster(Id); // renvoie tous les clusters
	auto biggest_cluster = all_cluster.begin(); // on initialise le max au premier
	for (auto itCluster = all_cluster.begin(); itCluster != all_cluster.end(); ++itCluster) { // on cherche le plus gros cluster
		if (itCluster->size() > biggest_cluster->size()) { // cluster plus gros
		...
		else if (itCluster->size() == biggest_cluster->size()) { // cluster de même taille
			unsigned int nbNeighA = 0; // compte les neighs de chacun
			unsigned int nbNeighB = 0;
			for (unsigned int id = 0; id < biggest_cluster->size(); ++id) {
				// on regarde donc lequel a le moins de voisind pour essayer d'aller en bord de map
			...
			if (nbNeighA < nbNeighB) { // si il a le moins de voisins cest le nouveau biggest_cluster
				biggest_cluster = itCluster;
			}
		}
	}
	...
	std::vector<unsigned int> cells_joinable{}; // contient toutes les cases que l'on doit essayer de rejoindre du plus gros cluster
	...
	// trouver tous les meilleurs chemins entre les autres clusters et les cells joignables
	...
	for (auto itCluster = all_cluster.begin(); itCluster != all_cluster.end(); ++itCluster) {
		if (itCluster != biggest_cluster) {
			for (auto& itcells : (*itCluster)) { // on parcourt les ids des cells de tous les clusters sauf le plus gros
				auto find = std::find_if(playableAttackable.begin(), playableAttackable.end(), [itcells](const std::pair<pSCell, std::vector<pSCell>>& attackplay) {return attackplay.first->infos.id == itcells; });
				if (find != playableAttackable.end()) { // le itcell est jouable
					informations best_path{ 999 }; // best_path contient un chemin demandant 999 des (pire scenario), supposé infini
					for (auto& itJoinable : cells_joinable) { // on parcourt toutes les cases que l'on a pour objectif
						// auto path = Pathfinding(informations(itcells, itJoinable, Map)); // recursif
						auto path = Pathfindingprim(itcells, itJoinable); // itératif
					...
					if (best_path.nb_dices != 0 && best_path.effective_path) { // on verifie que l'on trouve bien un chemin
	...
	if (path_to_keep.empty()) {
		...
		for (std::vector<std::vector<unsigned int>>::iterator itCluster = all_cluster.begin(); itCluster != all_cluster.end(); ++itCluster) {
			if (itCluster != biggest_cluster) {
				for (auto& itcells : (*itCluster)) { // on parcourt les ids des cells de tous les clusterd sauf le plus gros
		...
		// si il nous reste des coups à jouer
		if (!playableAttackable.empty()) {
			// on passe le relais à une autre stratégie
	...
	else { // un chemin pour rejoindre les clusters existe
		// sélection du chemin le moins couteux
...
{
	// prendre les cases faciles
	pSCell bestCellP = playableAttackable.begin()->first; // meilleure cellule depuis laquelle on attaque
	pSCell bestCellA = playableAttackable.begin()->second.operator[](0); // meilleure cellule à attaquer
	...
	for (auto& it : playableAttackable) { // parcourt tous les cases attaquables pour chaque case jouable
		for (auto& itAttack : it.second) {
			const int diff = it.first->infos.nbDices - itAttack->infos.nbDices;
			if (diff > bestDiff) { // trouve la case avec le plus grand écart de dés avec son adversaire
				bestDiff = diff;
				bestCellP = it.first;
				bestCellA = itAttack;
			}
	...
	if (bestDiff > 0) { // si la bestDiff est > à 0 on à des chances de remporter la case adverse donc on joue
...
{
	// attaquer seulement avec un stock de 8 dés ou plus à la fin du tour
	...
	if (future_dice_supply - missing_dices < 8) { // 8 car meme si on pert le duel on est sûr d'avoir de quoi revenir à 8 partout
	...
	pSCell bestCellA = playableAttackable.begin()->second.operator[](0); // meilleur cellule à attaquer
	int bestDiff = playableAttackable[0].first->infos.nbDices - playableAttackable[0].second.operator[](0)->infos.nbDices;// nb de dés de différence entre attaquant et attaqué
	for (auto& it : playableAttackable) { // parcourt tous les cases attaquables pour chaque case jouable
		for (auto& itAttack : it.second) {
...
{
	// détermination nouvelle strat
	// stratégie endgame se déclenche lorque le gain de dés est supérieur à tout ceux des adversaires ou dicestock > 8
	...
	if (index == Id || diceStock[Id] > 8) { // stratégie endgame si on a le joueur avec le plus de points, ou si on a plus de 8 stock en des
